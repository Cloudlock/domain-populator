...
LOCAL_LIBS = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'lib')

# do this twice, both here (for imports below) and in main() to always get the rest of the system set right
if LOCAL_LIBS not in sys.path:
    logging.info('adding to sys.path: %s', LOCAL_LIBS)
    sys.path.insert(1, LOCAL_LIBS)
else:
    logging.info('already have %s in sys.path', LOCAL_LIBS)
