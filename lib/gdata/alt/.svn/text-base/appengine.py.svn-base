...
#SR#  def request(self, operation, url, data=None, headers=None):
  def request(self, operation, url=None, data=None, headers=None):
    ...
    if isinstance(operation, atom.http_core.HttpRequest) and url is None:
      atomreq = operation

      operation = atomreq.method
      data = atomreq._body_parts
      url = str(atomreq.uri)
      headers = atomreq.headers
    ...
    ### TODO: Hack in a header:
    # GData-Version: 3.0
    # and see if that helps...
#    logging.info("=== getting url %s, method %s" % (str(url), str(method)))
#    logging.info("=== headers %r" % all_headers)
#    all_headers["GData-Version"] = "3.0"
    ...
    # SR hacked version with more logging
    if self.deadline is None:
      result = HttpResponse(urlfetch.Fetch(url=str(url), payload=data_str,
          method=method, headers=all_headers, follow_redirects=False))
    else:
      result = HttpResponse(urlfetch.Fetch(url=str(url), payload=data_str,
        method=method, headers=all_headers, follow_redirects=False,
    ...
    # bodysnippet_io = deepcopy(result.body)
    # logging.warn("=== got response %s, status=%s, body=%s" % (str(result), getattr(result, "status", None), bodysnippet_io.read(1000)))
    # logging.info("=== got response %s, status=%s" % (str(result), getattr(result, "status", None)))
    return result
...
class HttpResponse(object):
  ...
  def getheader(self, name, default_value=None):
    if not self.headers.has_key(name):
      return self.headers.get(name.lower(), default_value)
